...
// -> GUI
  ...
  void task_gui(void* param) { // -> main loop GUI
    initial_display();
    for (; ;) { // -> loop
      lv_tick_inc(5);
      lv_timer_handler();
  ...
  // -> window
    ...
    */
    ...
    void create_chart_window(lv_obj_t* scr) { // -> window chart
      ...
      } // -> тестовая серия для графика => нужно обновлять в реальном времени!
    ...
    **/
    ...
    const char* arr_data_recording[] = {
      "File System",
      "SD Card",
      "Google Sheets"
    };

    #define ALL_SCREEN_SETTINGS 3 // -> Общее кол-во экранов настроек (переключение экранов)


    lv_obj_t* create_tumbler(lv_obj_t* scr) { // -> создание тумблера для активации настроек
    ...
    void create_settings_window(lv_obj_t* scr) { // -> window settings
      ...
      lv_obj_t* change_panel_content = create_panel(scr, LV_ALIGN_TOP_MID, 320, 190, 0, 0);
      lv_obj_clear_flag(change_panel_content, LV_OBJ_FLAG_SCROLLABLE);
      lv_obj_set_scrollbar_mode(change_panel_content, LV_SCROLLBAR_MODE_OFF);

      lv_obj_t* main_lbl = create_label(change_panel_content, "Settings Window", LV_ALIGN_TOP_MID, 0, 0);

      for (int i = 0; i < 3; i++) {
        Serial.println(arr_data_recording[i]);
        create_label(change_panel_content, 
                     arr_data_recording[i], 
                     LV_ALIGN_TOP_LEFT, 10, (i + 1) * 32);
      } // -> возможный вариант, стоит пересмотреть


      /**

        void clear_labels(lv_obj_t* panel) { // -> в данном примере стоит вынести change_panel_content в глобальную область
          lv_obj_t* child = lv_obj_get_child(panel, NULL);

          while (child != NULL) {
            lv_obj_t* next = lv_obj_get_child(panel, child);
      ...
      */
      ...
      lv_obj_t* btn_next_settings = create_button(change_panel_content, LV_ALIGN_BOTTOM_LEFT, 40, 25, 0, 0);
      lv_obj_t* lbl_btn_next_settings = create_label(btn_next_settings, "Next", LV_ALIGN_CENTER, 0, 0);
      lv_obj_add_event_cb(btn_next_settings, handler_button, LV_EVENT_CLICKED, (void*) "Next Settings");

      lv_obj_t* btn_back_settings = create_button(change_panel_content, LV_ALIGN_BOTTOM_LEFT, 40, 25, 50, 0);
      lv_obj_t* lbl_btn_back_settings = create_label(btn_back_settings, "Back", LV_ALIGN_CENTER, 0, 0);
      lv_obj_add_event_cb(btn_back_settings, handler_button, LV_EVENT_CLICKED, (void*) "Back Settings");


      lv_obj_t* panel_menu_btn = create_panel(scr, LV_ALIGN_BOTTOM_MID, 320, 50, 0, 0);
      lv_obj_clear_flag(panel_menu_btn, LV_OBJ_FLAG_SCROLLABLE);
      lv_obj_set_scrollbar_mode(panel_menu_btn, LV_SCROLLBAR_MODE_OFF);

      lv_obj_t* btn_oven_window = create_button(panel_menu_btn, LV_ALIGN_LEFT_MID, 80, 50, 0, 0);
      lv_obj_t* lbl_btn_oven_window = create_label(btn_oven_window, "Oven", LV_ALIGN_CENTER, 0, 0);
      lv_obj_add_event_cb(btn_oven_window, handler_button, LV_EVENT_CLICKED, (void*) "Oven");

      lv_obj_t* btn_gas_window = create_button(panel_menu_btn, LV_ALIGN_CENTER, 80, 50, 0, 0);
      lv_obj_t* lbl_btn_gas_window = create_label(btn_gas_window, "Gas", LV_ALIGN_CENTER, 0, 0);
      lv_obj_add_event_cb(btn_gas_window, handler_button, LV_EVENT_CLICKED, (void*) "Gas");
      
      lv_obj_t* btn_chart_window = create_button(panel_menu_btn, LV_ALIGN_RIGHT_MID, 80, 50, 0, 0);
      lv_obj_t* lbl_btn_chart_window = create_label(btn_chart_window, "Chart", LV_ALIGN_CENTER, 0, 0);
      lv_obj_add_event_cb(btn_chart_window, handler_button, LV_EVENT_CLICKED, (void*) "Chart");
  ...
  // -> handler button
    static void handler_button(lv_event_t *e) {
      ...
      */
      ...
      if (strcmp(name, "Next Settings") == 0) {
        // -> logic handler btn

        Serial.println("Button - Next in window of settings is input");

      } else if (strcmp(name, "Back Settings") == 0) {

        Serial.println("Button - Back in window of settings is input");
